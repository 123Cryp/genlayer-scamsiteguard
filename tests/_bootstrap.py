...
Every test module in this package needs the same three things:
  ...
  3. A `make_contract()` helper that returns a fresh, empty
     ScamSiteGuard instance for each test.
...
_spec = importlib.util.spec_from_file_location("scamsiteguard_contract", _CONTRACT_PATH)
_contract_module = importlib.util.module_from_spec(_spec)
_spec.loader.exec_module(_contract_module)

ScamSiteGuard = _contract_module.ScamSiteGuard
gl = _contract_module.gl


def make_contract() -> "ScamSiteGuard":
    """
    Return a fresh ScamSiteGuard instance with empty storage.

    `ScamSiteGuard()` alone is sufficient: the offline stub's
